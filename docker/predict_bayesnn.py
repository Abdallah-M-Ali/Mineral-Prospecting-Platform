...
from BayesianNN.main_BayesNN import trainBayesNN
import os
...
def predict_bayesnn(data_all, output_dir, result_dir, batch_size=128, learning_rate=0.001):
    ...
    BNN_results = trainBayesNN(data_raw, label_raw, output_dir, batch_size=batch_size, learning_rate=learning_rate)

    # BayesNN outputs
    results['score'] = BNN_results['score']
    results['Confusion Matrix'] = BNN_results['Confusion Matrix']
    results['ACC'] = BNN_results['ACC']
    results['AUC'] = BNN_results['AUC']
    results['ROC'] = BNN_results['ROC']
    results['PR'] = BNN_results['PR']
    results['F1-score'] = BNN_results['F1-score']

    result_output_file_name = result_dir+'/report_BayesNN.csv'
    results['Report Path'] = result_output_file_name
    write_results(result_output_file_name, results, 'BayesNN', 'default', batch_size=batch_size, learning_rate=learning_rate)
...
if __name__ == '__main__':
    ...
    out = predict_bayesnn(sample, output_dir,result_dir,batch_size = batch_size,learning_rate = learning_rate)
    ...
    photo(x,y,score,output_dir_png,'Bayesnn')