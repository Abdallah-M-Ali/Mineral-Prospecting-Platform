# -*- coding: utf-8 -*-
"""
Created on Thu Jan  9 15:49:49 2020
...
def photo2(num,downs):
    plt.figure()
    text = []
    for i in range(len(num) - 1):
        text_seg = [i+1]
        downs_select = downs[downs[:, 0] > num[i]]
        ...
        plt.plot([downs_select[-1, 0], downs_select[-1, 0]],
                         [downs_select[-1, 1] - 0.3, downs_select[-1, 1] + 0.5], color='blue',
                         linewidth=2)

    plt.grid(linestyle='-.',which='major')
    labels = ['Seg','Log','value','K','b',r'$ R^2 $']
    text.insert(0,labels)
    my_table = plt.table(cellText = text,cellLoc = 'center',colWidths = [0.10]*6,
                       bbox = [0.03, 0.03, 0.8, 0.48])
...
# file_path = r'F:\mineral prospecting\C-A\Output\Part2\Npy\C_A.npy'
# point = '-0.2,1.75,2.25,2.75'
# result_output_plot = r'F:\mineral prospecting\C-A\Output\Part3\Plot\c-a.png'
#result_output_csv = r'F:\mineral prospecting\C-A\Output\Part3\Data\c-a.csv'
