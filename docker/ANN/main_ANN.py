...
from ANN.utils.BayesianModels.ANNWsNet import ANNWsNet
...
def train(net, optimizer, epoch, bs, data, label):
    ...
    for idx in range(numTrainBatch):
        ...
        loss = elbo(outputs, targets)
        loss.backward()
        optimizer.step()
        pred = torch.max(outputs, dim=1)[1]
        correct += torch.sum(pred.eq(targets)).item()
        total += targets.numel()
    print(f'[ANN  TRAIN] Acc: {100. * correct / total:.3f}, Iter: {iter_all:d}')
    return net
...
def test(net, optimizer, epoch, bs, data, label):
    ...
    with torch.no_grad():
        ...
        print(f'[ANN TEST] Acc: {accuracy:.3f}')


################################################################################################
################################################################################################
def trainANN(data_raw, label_raw, output_dir, batch_size=128, learning_rate=0.001):
    ...
    input_data = Variable(input_data)
    net.eval()
    scores_curves = net.forward(input_data)[:, 1].detach().numpy()
